...
# Introduction
In preparation for the launch of Limbo, the functionality of the existing generation of Pyrotokens was internally reviewed for compatibility with the requirements of Limbo and while there are no serious issues, there are aspects that prevent the Behodler ecosystem from reaching its full potential. This article explains what those aspects are and why this requires an upgrade to the Pyrotoken protocol. In designing a new set of Pyrotokens, an economic risk free form of yield generation for Pyrotoken reserves was discovered in the form of a unique loan dapp that could have significant ramifications for all of DeFi. Reserves on Pyrotokens2 were left intentionally inert to prevent importing external protocol or economic risk into the Pyrotoken protocol as this could undermine the redeem rate. PyroLoans addresses and solves these challenges and allows the protocol to earn economic risk free yield on reserves and at a competitive rate. This paper covers the following topics:

1. Pyrotokens are properly defined and a set of laws governing redeem rates is established.
...
The 3 laws of PyroTokens offer a solution to the above such that the underlying capital can be put to work without increasing the economic risk of the protocol at all. This opportunity cannot be passed up. The mechanism of yield is elaborated <a href ="#pyroLoan">in a subsequent section</a>.
...
Finally, while layer 2 appears to be the future of Ethereum dapps, V3 PyroTokens are still written such that if a Behodler 3 on mainnet were ever deployed, it could be done so with gas cost savings over the existing Behodler (version 2). In any event L2 code needs to still remain gas efficient as the existence of an additional layer does not eliminate gas supply and demand.
