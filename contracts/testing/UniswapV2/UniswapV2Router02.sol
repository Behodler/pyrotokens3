...
library UniswapV2Library {
    ...
    function getAmountOut(
        ...
        console.log(
            "getAmountOut: amountIn %d reserveIn %d reserveOut %d",
            amountIn,
            reserveIn,
            reserveOut
        );
    ...
    function getAmountsOut(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            console.log(
                "amounts[i] %d reserveIn %d reserveOut%d",
                amounts[i],
                reserveIn,
                reserveOut
            );
...
contract UniswapV2Router02 {
    ...
    function swapExactTokensForTokens(
        ...
        console.log("UNI: amounts[0] %d amounts[1] %s", amounts[0], amounts[1]);
        require(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 amountOutput;
            {
                ...
                (uint256 reserveInput, uint256 reserveOutput) = input == token0
                    ? (reserve0, reserve1)
                    : (reserve1, reserve0);
                amountInput = uniIERC20(input).balanceOf(address(pair)).sub(
                    reserveInput
                );
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            uniIERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >=
                amountOutMin,
            "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }
